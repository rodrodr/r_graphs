...
### The function mvioplot is a modified version of the original vioplot function of the package "vioplot" 
### created by Daniel Adler, and available at CRAN
### 
### Violin plots are a combination of a box plot and a probability density function plot. Together, these two graphs allow the
### analysis of the distribution of the data, its degree of dispersion/concentration, as well as the type and shape of the 
### probability distribution in a given set of cases.
### 
### The function extends the functionalities of the original package and allows the customization of some graphical parameters
### not available in the original code. The two main advancements are the parameters "by", allowing the automatic division of cases into groups,
### and "side", that allows to plot density functions on just one side of the graph   
### 
### For a clearer look and design, it also replaces polygons by lines in order to avoid visual overload.  
### We also added some style parameters to change the color of each graphical element (line color, width, type, 
### boxplot line color,type, width, boxplot rectangle color, etc.)


#### Function mvioplot() - multivariate violin plot. 
#### Parameters:
...
####            drawRect - defines if the boxplot will be ploted or not. This is useful when users want to draw more than
####                        one density function at the same side of the violin plot. This allows the comparison of multiple
####                        density functions in one graph.
####            side - the side of the plot to draw the density function lines. The possible values are "both","hboth" - horizontal,
####                   left, right, up, down. When users define "hboth", "up", or "down", the function automatically changes the 
####                   orientation to horizontal. The default value for this parameter is "both".
####            box - designs a box around the violin plot. The default is false.
####            lag - defines the space from the middle of the violin plot. It is useful to avoid the juxtaposition of boxplots when
####                  users are employing two different variables in each side of the violin plot. The default value is 0.03.
...
# The adapted code for the function
...
# Create some data using Brazilian Atlas of Human development of 2013
...
mvioplot(d$HDI.M , d$year, side="left", lag=0.04, ylim=c(0,1), lwd=2, horizontal = T)
mvioplot(d$GINI , d$year, side="right", lag=0.04, col="orange", rectCol = "#F7BE81", col.line = "orange", rectBord = "orange",colMed = "orange", add=T, lwd=2, , horizontal = T)


legend("topleft", fill=rep("white",2), border = c("black","orange"), legend = c("Human Development Index", "Gini Index"), bty = "n", horiz = F, x.intersp = 0.2)
